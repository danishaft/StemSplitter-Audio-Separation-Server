...
def _headers() -> dict[str, str]:
    api_key = os.getenv("TUSS_WORKER_API_KEY")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Smoke-test TUSS worker.")
    parser.add_argument("input", type=Path)
    parser.add_argument("--job-id", default="tuss_worker_smoke")
    parser.add_argument("--out-dir", type=Path, default=ROOT / "jobs" / "tuss_worker_smoke")
    parser.add_argument("--seconds", type=float, default=2.0)
    parser.add_argument("--sample-rate", type=int, default=48000)
    args = parser.parse_args()

    base_url = os.getenv("TUSS_WORKER_URL")
    if not base_url:
        raise SystemExit("TUSS_WORKER_URL is not configured")
    base_url = base_url.rstrip("/") + "/"

    args.out_dir.mkdir(parents=True, exist_ok=True)
    input_clip = args.out_dir / "input" / "tuss-smoke-input.wav"
    ...
    report_path = args.out_dir / "tuss_worker_smoke_report.json"
